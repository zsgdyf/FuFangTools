  ...
  <div class="problem-container">
    ...
    <section class="solution-section" style="margin-bottom: 2rem;">
      ...
      <div v-else class="step-card official">
        <h3>✨ 突破极限：面积为什么能压缩到 0？</h3>
        <p>
          鲍尔平移（Páll's shift）本身只是把一个三角形切成两半并重叠，面积只是稍微变小了一点。真正的奇迹在于基于它的<strong>无限次递归细分（构造佩龙树）</strong>：
        </p>
        <ol>
          <li><strong>无限切分</strong>：假设我们将三角形从顶点到底边切分成 $2^n$ 个极细的“小三角形”。原本它们的底边排开总长是 $b$。</li>
          <li><strong>树干与树冠</strong>：通过鲍尔平移，我们强制让这 $2^n$ 个小三角形的底边相互滑动并<strong>高度重叠</strong>，形成一个极细的“树干”。此时树干的宽度骤降为 $\frac{b}{2^n}$，这部分的面积几乎为 $0$。</li>
          <li><strong>角度不丢失</strong>：与此同时，这 $2^n$ 个三角形的顶点为了覆盖原本的所有方向，依然像树枝一样向外散开，形成“树冠”。针依然可以在每一根极细的树枝里完成微小的角度旋转，再通过树干平移到下一根树枝继续旋转。</li>
          <li><strong>神奇的极限</strong>：直觉上树冠似乎会占据很大面积，但严格的数学证明指出，只要每次平移重叠得足够精妙，由于重叠部分的面积远大于树冠尖端不重叠碎片的面积，<strong>当细分次数 $n \to \infty$ 时，整棵“佩龙树”的总面积会趋近于极限 $0$！</strong></li>
        </ol>
        <p>
          这就是挂谷猜想（贝西科维奇集）最违反直觉的魅力：<strong>你可以拥有所有方向的线段，但你不需要为它们支付任何“面积”作为代价！</strong>
        </p>
      </div>
    </section>
